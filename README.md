...
| Block 3 | 2x Dockerfiles + nginx.conf + docker-compose | ~45 min |
...
- **Tool used:** Kiro (AI-assisted IDE).
- **How it was used:** As a coding assistant for implementation and documentation.
  Block selection, tool choices, and security decisions were directed by me; AI
  output was reviewed and corrected against real build/test/scan feedback.
- **Key decisions and fixes I directed:**
  ...
  - Every fix was verified by re-running the relevant build/test locally before
    commit (the npm-audit changes, with no local Node.js, were validated by the pipeline).
...
|---------|----------------|-------------------|
| Multi-stage build | Yes JDK -> JRE | Yes Node -> Nginx |
| Non-root user | Yes UID 1001 | Yes nginx (UID 101) |
| Read-only filesystem | Yes + tmpfs /tmp | Yes + tmpfs cache/run/tmp |
| Drop all capabilities | Yes cap_drop: ALL | Yes cap_drop: ALL |
| No privilege escalation | Yes | Yes |
| Security headers | n/a | Yes 7 headers + CSP |
| Health check | Yes | Yes |
| Resource limits | 512 MB / 1 CPU | 128 MB / 0.5 CPU |
| Secrets via env only | Yes | Yes |
